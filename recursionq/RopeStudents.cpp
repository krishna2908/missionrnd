...
int len(struct student* root)
{
	if (root == NULL)return 0;
	return  max(max(1 + len(root->first), 1 + len(root->second)), 1 + len(root->third));
}
void depth(struct student* root,int *m,int d)
{
	if (root == NULL)return;
	depth(root->first, m, d + 1);
	depth(root->second, m, d + 1);
	depth(root->third, m, d + 1);
...
{
	*maxBallDistance= len(leaderStudent);
	*minBallDistance = 19191919;
	depth(leaderStudent,minBallDistance,1);
	return;
}
